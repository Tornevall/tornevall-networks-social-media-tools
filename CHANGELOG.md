...
### Changed
- Facebook participant-request **Analyze user** now also captures structured original-post comment threads from Facebook GraphQL responses such as `CometPhotoRootContentQuery` after the operator opens the preview/image/original-post flow. The detected-context list and preview debug box now show explicit thread-comment counts, original-post text, and comment excerpts instead of leaving those later comments hidden behind generic network snippets only.
- **Verify fact** no longer chooses its own model inside the extension. Tools now decides the verification model server-side, and verify runs can prefer `gpt-4o` when web search is the active verification path.
...
- Facebook participant-request preview follow-up now keeps rechecking a clicked preview/original-post dialog for several extra seconds, so slower Facebook loads are less likely to miss newly opened **View original post** content after the first quick DOM hit.
- Facebook participant-request moderation-history rendering no longer crashes on the missing `getParticipantHistoryForSummary` / participant-history helper chain. Earlier approved/rejected history can now be looked up and rendered again without the content script stopping on a `ReferenceError`.
- Facebook participant-request card detection now also uses the in-card Facebook profile URLs (`/groups/<id>/user/...`) as a fallback signal and climbs higher through Facebook's deeper moderation markup, so request cards are less likely to disappear just because button wrappers or parent nesting changed again.
...
- The participant helper now again exposes direct **Open Toolbox** and **Verify fact** actions both inline on detected request cards and from the floating fallback list, instead of leaving only the newer **Analyze user** flow available.
- Facebook participant-request user analysis no longer crashes on the missing `collectParticipantContextNeighborhoodLines` helper. Preview/context follow-up parsing now has that neighborhood collector again, so the participant helper can keep building preview-aware context without throwing a `ReferenceError`.
- Visible Facebook participant-request cards can now query Tools for earlier approved/rejected moderation history from linked logged groups, show that history as a compact inline badge on the card, and feed the same summary back into **Analyze user** so previous approvals/rejections can influence the analysis context.
- Facebook participant-request preview tracking now keeps following the same matched preview dialog after the first hit, so later preview updates — including when the operator clicks through to **View original post** inside that preview — can still extend the analysis context instead of being treated like an unrelated surface.
- Preview-dialog context extraction is now more participant-name-aware, so scattered comment/post lines that belong to the analyzed person are prioritized more strongly while generic preview UI noise is filtered harder.
- Participant-request user analysis now races two backend lookups in parallel — one without web search and one with web search — so the fastest useful answer can be shown first both on the initial visible-card pass and on later preview/original-post follow-up passes. Later richer results can still replace the first answer when they add more verified value.
- `Find preview element` now also forces another preview/comment rescan after it focuses the matched dialog, which helps the helper catch newly expanded full-thread context when Facebook's DOM-change signals lag behind.
- The first visible-card-only participant analysis still stays on a faster no-web-search path by default when it is not running in that explicit race mode, while later participant-preview follow-up reruns can still use web search again when the richer comment/original-post context contains claims worth checking. Those follow-up reruns also give the participant's own visible comment extra analytical weight against the saved group rules/context, and the waiting notice now backs off more gracefully when Facebook has already stopped changing instead of sticking to one rigid “wait a few more seconds” message.
