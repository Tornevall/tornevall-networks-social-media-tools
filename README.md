...
Verify-fact note: when the backend decides that a check requires independent verification, Tools now treats the normal AI answer and the web-search verification lookup as separate steps. The preliminary answer can still be shown if web search fails or adds no useful evidence, but the metadata clearly marks the result as not independently verified. Verify-mode model selection is now Tools-controlled as well, so the extension no longer chooses a dedicated fact-check model locally; when web search is part of the run, Tools may prefer `gpt-4o` as the primary verification model. Fact/user-analysis result boxes now render the first markdown answer as safe HTML (headings, bold text, lists, and links), can show structured citation links returned by Tools, and use an internal scroll area so longer source-backed answers remain readable without hiding the action buttons. The first pending **Analyzing user…** / **Verifying facts…** state is also styled more cleanly with a compact preview card instead of a flattened text dump.
...
The same Facebook participant-request detector is now also less brittle when Meta changes the moderation-card wrappers again: besides the existing approve/reject/button heuristics, it now treats the in-card Facebook profile URLs (`/groups/<id>/user/...`) as a strong card identity signal and climbs farther through deep nested wrappers before giving up. That makes the helper more likely to keep recognizing the real per-user card even when the surrounding DOM shifts.
...
When the operator clicks through from that preview into the underlying image/original-post view, the helper can now also capture the later GraphQL thread payloads (for example `CometPhotoRootContentQuery`) as structured original-post comment context. The floating **Detected context** list and the expandable preview debug box now show explicit thread-comment counts, original-post text, and captured comment excerpts so it is easier to confirm that the deeper comment thread really made it into **Analyze user**.
...
If you leave the Facebook `participant_requests` page, the floating participant user-analysis box now closes automatically and clears any stale follow-up state instead of lingering over unrelated pages.

When a visible participant name/profile matches earlier moderation rows already logged through the separate Facebook admin-activity ingest, the helper now also asks Tools for that linked history and surfaces it directly on the request card as a compact earlier-decisions badge (for example previous approvals or rejections from linked logged groups). The same history summary is also injected into the participant-analysis context so **Analyze user** can weigh earlier accepted/rejected decisions together with the current visible card/preview clues.
...
- visible participant-request cards are detected from the live Facebook DOM, and the scan now falls back to a broader whole-page action walk again when Facebook's participant-request root or attachment anchor gets too strict
- each matched card now keeps compact **Analyze user**, **Open Toolbox**, and **Verify fact** actions, while the floating fallback list still exposes **Show card**
...
- **Analyze user** now opens a verify-style result box instead of the full Toolbox composer so the participant review stays focused on user analysis first, while the restored **Open Toolbox** button gives you the older direct-composer route again when you want it
